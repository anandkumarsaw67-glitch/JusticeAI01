...
// Image chat endpoint - analyze images
...
    return res.json({ reply: 'Please provide an image.' });
...
    // For demo, we'll simulate image analysis. In real implementation, call AI vision API



    const reply = await analyzeImage(image, filename);
...
    res.json({ reply: 'Sorry, I encountered an error analyzing the image. Please try again.' });
...
// Function to analyze images using AI APIs



async function analyzeImage(image, filename) {
...
    // Try Grok vision API first



    const grokResponse = await callGrokVisionAPI(base64Data, filename);
...
    console.log('Grok Vision API failed:', error.message);
...
    return "I've analyzed your image. This appears to be a document/image that you'd like me to review. For a detailed legal analysis, please also describe the content or context of this image so I can provide more specific guidance.";
...
async function callGrokVisionAPI(imageData, filename) {
...
              text: 'Analyze this image and provide legal insights or guidance if applicable. Describe what you see in the image.'
